...
/////////////////////////////////////////
int a2i(char* t_imei);
static void Send_Bttry_Data(void);
char Imei_m[15]="";
char pswrd_string[27]="";   //
...
/************************************************************************/
static u8  m_SrvADDR[20] = "13.234.46.70\0";
...
void proc_main_task(s32 taskId)
{
    ...
    ret = Ql_UART_Open(UART_PORT1, 9600, FC_NONE);       //    9600 can be placed here 
    if (ret < QL_RET_OK)
    {
...
///////////////////////////// testing /////////////////////////////////////////////////////////
                   ...
                   // APP_DEBUG("IMEI=%s\r\n",read_imei);
                   // imei_pswrd = a2i(read_imei);
                   
                   // APP_DEBUG("\npswrd=%d\r\n",imei_pswrd);
                    //APP_DEBUG("\nImei_m=%s  len=%d",Imei_m,Ql_strlen(Imei_m));

                   // Ql_sprintf(pswrd_string,"@$,1,%s,%d,#",Imei_m,imei_pswrd);
                   // APP_DEBUG("\npswrd_string_sprint=>%s",pswrd_string);                   					
////////////////////////////////////////////////////////////////////////////////////////////////// 	
...
int a2i(char* t_imei)
{
    ...
    s32 len=Ql_strlen(t_imei);
    APP_DEBUG("t_imei_len=%d",len);
    for (i = 0; i < len; i++)
     {
         if((t_imei[i]<=0x39) && (t_imei[i]>=0x30))
         {
        Imei_m[j]=t_imei[i];
        digit = t_imei[i] - 0x30;
        sum = sum  + digit;
    ...
    case EVENT_UART_READY_TO_READ:
        {
           ...
           if (totalBytes > 0)
           {
              ...
              APP_DEBUG("data recieved=%s",m_RxBuf_Uart);
           }
           break;
        }
...
static void GPRS_TCP_Program(void)
{
    ...
    //7. Send data to socket
    {
        ...
        int imei_pswrd=0;  // stored pswrd int form
        s32  dataLen = 0;
        u64  ackNum = 0;
        Ql_memset(pchData, 0x0, sizeof(pchData));
        RIL_GetIMEI(read_imei);
        imei_pswrd = a2i(read_imei);
        dataLen = Ql_sprintf(pchData + dataLen,"@$,1,%s,%d,#",Imei_m,imei_pswrd );
...
/*static void Send_Bttry_Data(void)
{
        ...
        s32  dataLen = 0;
        u64  ackNum = 0;
        Ql_memset(Bttry_Data, 0x0, sizeof(Bttry_Data));
        // RIL_GetIMEI(read_imei);
        //imei_pswrd = a2i(read_imei);
        dataLen = Ql_sprintf(Bttry_Data + dataLen,"@$,1,%s,%d,#",m_RxBuf_Uart);
        APP_DEBUG("<-- Sending data(len=%d): %s-->\r\n", dataLen, Bttry_Data);
        ret = Ql_SOC_Send(m_SocketId, (u8*)Bttry_Data, dataLen);
        if (ret == dataLen)
        {
            APP_DEBUG("<-- Send socket data successfully. --> \r\n");
        }
...
static void Callback_Socket_Read(s32 socketId, s32 errCode, void* customParam )
{
    ...
    do
    {
        ...
        else if(SOC_WOULDBLOCK == ret)  // Read finish
        {
            ...
            ch=Ql_strstr((char*)m_SocketRcvBuf,"##");
            APP_DEBUG("pointer=%d",ch);
            //############################  testing  ###########################################
            //############################  testing  ###########################################
            if( ch!=NULL)
            {
                APP_DEBUG("Response Recieved--> %s\n",m_SocketRcvBuf);
               
            }
            else
            {
                 APP_DEBUG("Incorrect Response-->%s\n", m_SocketRcvBuf);
            }
