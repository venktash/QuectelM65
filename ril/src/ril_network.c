...
static s32 ATResponse_CREG_Handler(char* line, u32 len, void* userdata)
{
    ...
    if(head)
    {
        ...
        if(RIL_Parse_value(head, &n, state))
        {
            //Ql_sscanf(head,"%*[^ ]%d,%d,%[^\r\n]",&n,state);
...
static s32 ATResponse_CSQ_Handler(char* line, u32 len, void* userdata)
{
    ...
    if(head)
    {
        if(RIL_Parse_value(head, &CSQ_Reponse->rssi, &CSQ_Reponse->ber))
        {
            return  RIL_ATRSP_CONTINUE;
        }
...
bool RIL_Parse_value(s8* pData,s32* value1, s32* Value2)
{
    bool retVal = TRUE;
    
   //#################################### begin param1 ################
    //first trim the none digtal value
    *value1 =0;
    *Value2 =0;
    while(pData)
    {
      ...
      pData++;//if not digtal plus 
    }
    
    //there is no digtal value then return FALSE
    if(!pData)
    {
        return FALSE;
    }
...
//####################################begin param  2################
   //first trim the none digtal value
    ...
    //there is no digtal value then return FALSE,only parse 1 param return FALSE 
    if(!pData)
    {
        return FALSE;
    }
